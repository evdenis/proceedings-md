...
export function convertMetaToObject(meta: any): any {
    let result = {}
...
export async function markdownToJson(markdown: string): Promise<any> {
    let json = await pandoc(markdown, ["-f", "markdown", "-t", "json", ...pandocFlags])
    return JSON.parse(json)
}

export async function jsonToDocx(ast: any, target: string): Promise<void> {
